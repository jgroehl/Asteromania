	...
	public List<Killable> getLevelObjects(Context context, int level)
	{
		...
		switch (getLevelType(level))
		{
			...
			case BOSS:
				...
				break;
			case MIXED:
				numberOfEnemies = random((level + 1.0) / 1.5 * 0.9, (level + 1.0) / 1.5 * 1.1);
				...
				numberOfEnemies = random(Math.cbrt(level) * 0.9, Math.cbrt(level) * 1.1);
			...
			case NORMAL:
			default:
				numberOfEnemies = random(Math.sqrt(level) * 0.9, Math.sqrt(level) * 1.1);
				if (numberOfEnemies < 1)
					numberOfEnemies = 1;
