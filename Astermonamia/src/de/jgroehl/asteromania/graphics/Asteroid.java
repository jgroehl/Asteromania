...
public class Asteroid extends GraphicsObject implements Updatable, Hitable, Killable
{
	...
	{
		SMALL(0.4f, de.jgroehl.asteromania.R.drawable.rock3, 0.05f), MEDIUM(0.8f,
				de.jgroehl.asteromania.R.drawable.rock5, 0.07f), LARGE(1.2f, de.jgroehl.asteromania.R.drawable.rock4,
				0.09f), HUGE(1.7f, de.jgroehl.asteromania.R.drawable.rock2, 0.11f), EXTREME(2.8f,
				de.jgroehl.asteromania.R.drawable.rock1, 0.13f);
	...
	private Asteroid(float xPosition, float yPosition, int graphicsId, float relativeWidth, Context context, int life,
	...
	public void update(BaseGameHandler gameHandler)
	{
		if (gameHandler instanceof AsteromaniaGameHandler)
		{
			...
			if (imagesOverlap(getX(), getY(), relativeWidth, relativeHeight, asteromaniaGameHandler.getPlayer().getX(),
					asteromaniaGameHandler.getPlayer().getY(), asteromaniaGameHandler.getPlayer().getRelativeWidth(),
					asteromaniaGameHandler.getPlayer().getRelativeHeight()))
			{
				asteromaniaGameHandler.getPlayer().getHitByAsteroid(asteromaniaGameHandler, damage);
	...
	public static Asteroid createAsteroid(Context context, int level)
	{
		...
		return new Asteroid((float) Math.random() * (1 - type.getWidth()),
				(float) (Math.random() * (-(level / 4.0))) - 0.3f, type.getImage(), type.getWidth(), context,
				(int) Math.round(BASIC_LIFE * type.getModifyer() * Math.sqrt(level)), (int) Math.round(BASIC_DAMAGE
						* type.getModifyer() * Math.sqrt(level)), BASIC_SPEED / (float) Math.cbrt(type.getModifyer()));
	}

	private static AsteroidType getRandomAsteroidType()
	{
		return AsteroidType.values()[(int) (Math.random() * AsteroidType.values().length)];
	}
